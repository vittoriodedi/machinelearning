...
class CatMouseCheeseEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if (0 <= new_mouse_pos[0] < self.grid_size and 0 <= new_mouse_pos[1] < self.grid_size):
        ...
        for cat_action in range(4):
            ...
            if (0 <= new_cat_pos[0] < self.grid_size and 0 <= new_cat_pos[1] < self.grid_size):
...
def render_entities(window, mouse_pos, cat_pos, cheese_pos, grid_size, mouse_img, cat_img, cheese_img):
...
def test_q_learning(env, q_table, episodes=10, delay=0.5):
    ...
    for i in range(episodes):
        ...
        while not done:
            ...
            render_entities(screen, env.mouse_pos, env.cat_pos, env.cheese_pos, env.grid_size, mouse_img, cat_img, cheese_img)
            pygame.display.flip()
            clock.tick(30)
            time.sleep(delay)
